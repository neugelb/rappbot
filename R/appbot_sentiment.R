...
#' @param method The sentiments method you want to check out - possibilities are 'breakdwown' and 'timeline'
...
appbot_sentiment <- function(app_id,method,start=NULL,end=NULL,country=NULL,sentiments=NULL,dlangs=NULL,version=NULL,topic=NULL) {
  ...
  if (!str_to_lower(method) %in% c('breakdown','timeline')) {
...
url <- paste(url_start,app_id,'sentiment',method,sep='/')
...
sentiments <- do.call('bind_rows',map(call_content$results,as_tibble))

if (method == 'breakdown') {

  sent_names <- tibble(sentiment_type = c('positive','negative','neutral','mixed'))

  sentiments <- bind_cols(sent_names,sentiments) %>%
    rename(count = 2)
