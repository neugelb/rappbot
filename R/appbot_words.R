#' appbot_words
#'
#' Allows you to call different Appbot API endpoints related to keywords
#' @param app_id The ID of the app you want to check
#' @param method The words method you want to check out - possibilities are 'popular', 'critical', 'interesting', 'new', 'trending_up', and 'trending_down'
...
appbot_words <- function(app_id,method,start=NULL,end=NULL,country=NULL,sentiments=NULL,dlangs=NULL,version=NULL,topic=NULL) {
  ...
  if (!method %in% c('popular', 'critical', 'interesting', 'new', 'trending_up', 'trending_down')) {
...
url <- paste(url_start,app_id,'words',method,sep='/')
...
words <- do.call('bind_rows',map(call_content$results,as_tibble))
